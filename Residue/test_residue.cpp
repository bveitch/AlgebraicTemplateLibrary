...
bool sanity_tests(){
	cout << "Sanity tests for modulus" << endl;
	typedef Residue<int,13> Mod;
	int k11=11;
	int k09=22;
	int k06=71;
...
template<class T, unsigned int Prime>
bool test_Residue(ofstream& file,T a_, T b_, T c_)
{
	typedef Residue<T,Prime> Modulus;
	Modulus a(a_);
	Modulus b(b_);
	Modulus c(c_);
...
int main (int argc, char *argv[]) { 
	...
	int b(randfn(gen));
	int c(randfn(gen));
	ofstream file("test_Residue.txt");
	static constexpr unsigned int P = 31;
	pass*=test_Residue<int,P>(file,a, b, c);
	if(pass){
